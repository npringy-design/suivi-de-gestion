...
import { useData, DayDataAmexAncv } from './DataContext';
...
export default function AmexAncv({ month, year, onBack }: AmexAncvProps) {
  ...
  const handleDataChange = (day: number, field: keyof DayDataAmexAncv, value: string) => {
    updateAmexAncv(month, day, field, value);
  };
  ...
  const getVal = (day: number, field: keyof DayDataAmexAncv) => {
    const val = parseFloat((data[day]?.[field] || '').replace(',', '.'));
    return isNaN(val) ? 0 : val;
  };

  const getTheoriqueVal = (day: number, field: 'amex') => {
    const val = parseFloat((theoriqueData[day]?.[field] || '').replace(',', '.'));
    return isNaN(val) ? 0 : val;
  };

  const getColTotal = (field: keyof DayDataAmexAncv) => {
    return days.reduce((sum, day) => sum + getVal(day, field), 0);
  };

  const getTheoriqueColTotal = (field: 'amex') => {
    return days.reduce((sum, day) => sum + getTheoriqueVal(day, field), 0);
  };

  const sumTheorique = getTheoriqueColTotal('amex');
  const sumReel = getColTotal('reel_nepting');
            ...
            <table className="w-full text-sm text-left">
              <thead className="text-xs text-slate-500 uppercase bg-slate-50 border-b border-slate-200 sticky top-0 z-20">
                <tr>
                  ...
                  <th className="px-6 py-4 font-bold text-center w-48 bg-blue-50/50 border-r border-slate-200 text-blue-700">AMEX / ANCV Théorique</th>
                  <th className="px-6 py-4 font-bold text-center w-48 bg-amber-50/50 border-r border-slate-200 text-amber-700">Réel Nepting</th>
                  <th className="px-6 py-4 font-bold text-center w-48 border-r border-slate-200">Total</th>
                  <th className="px-6 py-4 font-bold text-center w-48 border-r border-slate-200">Écart</th>
                  <th className="px-6 py-4 font-bold min-w-[300px]">Commentaire</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {days.map((day) => {
                  const dayData = data[day] || { reel_nepting: '', commentaire: '' };
                  
                  const theorique = getTheoriqueVal(day, 'amex');
                  const reel = getVal(day, 'reel_nepting');
                    ...
                    <tr key={day} className="hover:bg-slate-50/50 transition-colors group">
                      ...
                      <td className="px-3 py-2 border-r border-slate-100">
                        <div className="bg-amber-50/50 rounded-lg border border-amber-100/50 group-hover:border-amber-200 transition-colors">
                          <CurrencyInput 
                            value={dayData.reel_nepting} 
                            onChange={(val) => handleDataChange(day, 'reel_nepting', val)} 
