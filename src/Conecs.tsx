...
export default function Conecs({ month, year, onBack }: ConecsProps) {
  ...
  const handleDataChange = (day: number, field: keyof DayData, value: string) => {
    updateConecs(month, day, field, value);
  };
  ...
  const getTheoriqueVal = (day: number, field: 'tr_carte') => {
  ...
  const getTheoriqueColTotal = (field: 'tr_carte') => {
    return days.reduce((sum, day) => sum + getTheoriqueVal(day, field), 0);
  };

  const sumConecsTheorique = getTheoriqueColTotal('tr_carte');
  const sumConecsReel = getColTotal('conecs_reel_nepting');
  
  const sumTotalConecs = sumConecsReel;
  const sumTotalEcart = sumConecsReel - sumConecsTheorique;
            ...
            <table className="w-full text-sm text-left">
              <thead className="text-xs text-slate-500 uppercase bg-slate-50 border-b border-slate-200 sticky top-0 z-20">
                <tr>
                  ...
                  <th className="px-4 py-4 font-bold text-center w-32 bg-blue-50/50">Conecs Théorique</th>
                  <th className="px-4 py-4 font-bold text-center w-32">Conecs Réel Nepting</th>
                  <th className="px-4 py-4 font-bold text-center w-32">Total Conecs</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {days.map((day) => {
                  const dayData = data[day] || { conecs_reel_nepting: '', commentaire: '' };
                  
                  const conecsTheorique = getTheoriqueVal(day, 'tr_carte');
                  const conecsReel = getVal(day, 'conecs_reel_nepting');
                  
                  const totalConecs = conecsReel;
                  const ecart = conecsReel - conecsTheorique;
              ...
              <tfoot className="bg-slate-800 text-white font-bold sticky bottom-0 z-20">
                <tr>
                  ...
                  <td className="px-4 py-4 text-center text-blue-200">{formatCurrency(sumConecsTheorique)}</td>
                  <td className="px-4 py-4 text-center text-amber-200">{formatCurrency(sumConecsReel)}</td>
                  <td className="px-4 py-4 text-center">{formatCurrency(sumTotalConecs)}</td>
