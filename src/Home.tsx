...
export default function Home() {
  ...
  const loadWeather = useCallback(async () => {
    ...
    try {
      const res = await fetch('https://api.open-meteo.com/v1/forecast?latitude=48.8755&longitude=2.7467&current_weather=true&daily=weathercode,temperature_2m_max,temperature_2m_min,precipitation_probability_max&forecast_days=7&timezone=Europe%2FParis');
  ...
  const loadDayEvent = useCallback(async (date: Date) => {
    ...
    try {
      ...
      const res = await fetch(`https://nominis.cef.fr/json/saintdujour.php?${params.toString()}`);
      if (!res.ok) return fallback;

      const payload = await res.json();
      const saintDuJour = payload?.response?.saintdujour?.nom;

      if (typeof saintDuJour === 'string') {
        return cleanDayEventLabel(saintDuJour) || fallback;
      }

      return extractDayEventFromNominis(payload) || fallback;
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const refreshDayEvent = async () => {
      ...
      if (isActive) setDayEventLabel(label);
    };

    refreshDayEvent();
    const interval = window.setInterval(refreshDayEvent, 60 * 60 * 1000);
  ...
  const dayEvent = dayEventLabel || getDayEvent(today);
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gradient-to-br from-slate-50 via-blue-50/30 to-slate-50">
                  ...
                  <div className="w-full sm:w-auto">
                    <div className="flex flex-col sm:flex-row items-stretch gap-3">
                      ...
                      <div
                        ...
                        className="min-w-[170px] rounded-[20px] border border-slate-200 bg-white px-4 py-3 shadow-md"
                      >
      ...
      {isForecastOpen && (
        ...
        >
          <div
            className="w-full max-w-5xl overflow-hidden rounded-[24px] border border-cyan-100/20 bg-gradient-to-br from-slate-950 via-blue-950 to-cyan-950 shadow-2xl"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between border-b border-cyan-100/15 px-5 py-4">
              <div>
                <div className="text-lg font-black text-amber-50">Prévisions météo 7 jours</div>
                <div className="text-xs font-semibold text-cyan-50/70">Montévrain</div>
              </div>
              <button
                type="button"
                onClick={() => setIsForecastOpen(false)}
                className="flex h-9 w-9 items-center justify-center rounded-full bg-white/10 text-cyan-50 transition hover:bg-white/20"
            ...
            <div className="grid max-h-[70vh] gap-2 overflow-y-auto p-4 sm:grid-cols-2 lg:grid-cols-7">
              {forecastDays.length > 0 ? (
                forecastDays.map((day, index) => (
                  <div
                    key={day.date}
                    className="rounded-2xl border border-cyan-100/15 bg-white/10 p-3 text-center shadow-inner shadow-black/10"
                  >
                    <div className="text-[11px] font-black uppercase tracking-[0.08em] text-cyan-50/80">
                      {index === 0 ? "Aujourd'hui" : formatForecastDate(day.date)}
                    </div>
                    <div className="mx-auto mt-3 flex h-12 w-12 items-center justify-center rounded-2xl bg-white/10 ring-1 ring-white/10">
                      {getWeatherIcon(day.code, 'w-8 h-8')}
                    </div>
                    <div className="mt-3 min-h-[28px] text-[11px] font-semibold leading-tight text-cyan-50/80">
                      {getWeatherLabel(day.code)}
                    </div>
                    <div className="mt-3 text-lg font-black text-amber-50">
                      {Math.round(day.max)}°
                    </div>
                    <div className="text-xs font-bold text-cyan-50/60">
                      min {Math.round(day.min)}°
                    </div>
                    <div className="mt-2 rounded-full bg-cyan-100/10 px-2 py-1 text-[10px] font-bold text-cyan-50/75">
                      Pluie {day.precipitation ?? 0}%
                    </div>
                  </div>
                ))
              ) : (
                <div className="col-span-full rounded-2xl border border-cyan-100/15 bg-white/10 p-6 text-center text-sm font-semibold text-cyan-50/80">
