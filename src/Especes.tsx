...
export default function Especes({ month, year, onBack }: EspecesProps) {
  ...
  const handleDataChange = (day: number, field: keyof DayData, value: string) => {
    updateEspeces(month, day, field, value);
  };
  ...
  const getTheoriqueVal = (day: number, field: 'especes') => {
  ...
  const getTheoriqueColTotal = (field: 'especes') => {
    return days.reduce((sum, day) => sum + getTheoriqueVal(day, field), 0);
  };

  const sumEspecesTheorique = getTheoriqueColTotal('especes');
  const sumMisAuCoffre = getColTotal('mis_au_coffre');
  const sumPieces = getColTotal('pieces');
  
  const sumTotalEcart = sumMisAuCoffre + sumPieces - sumEspecesTheorique;
            ...
            <table className="w-full text-sm text-left">
              <thead className="text-xs text-slate-500 uppercase bg-slate-50 border-b border-slate-200 sticky top-0 z-20">
                <tr>
                  ...
                  <th className="px-4 py-4 font-bold text-center w-32 bg-blue-50/50">Espèces Théorique</th>
                  <th className="px-4 py-4 font-bold text-center w-32">Mis au Coffre</th>
                  <th className="px-4 py-4 font-bold text-center w-32">Pièces</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {days.map((day) => {
                  const dayData = data[day] || { mis_au_coffre: '', pieces: '', commentaire: '' };
                  
                  const especesTheorique = getTheoriqueVal(day, 'especes');
                  const misAuCoffre = getVal(day, 'mis_au_coffre');
                  const pieces = getVal(day, 'pieces');
                  
                  const ecart = misAuCoffre + pieces - especesTheorique;
                  ...
                  return (
                    <tr key={day} className="hover:bg-slate-50/50 transition-colors group">
                      ...
                      <td className="px-4 py-2 text-center font-semibold text-slate-700 bg-blue-50/30">
                        {especesTheorique !== 0 ? formatCurrency(especesTheorique) : '-'}
                      </td>
                      <td className="px-2 py-2">
                        <div className="bg-amber-50/50 rounded-lg border border-amber-100/50 group-hover:border-amber-200 transition-colors">
                          <CurrencyInput 
                            value={dayData.mis_au_coffre} 
                            onChange={(val) => handleDataChange(day, 'mis_au_coffre', val)} 
                      ...
                      <td className="px-2 py-2">
                        <div className="bg-amber-50/50 rounded-lg border border-amber-100/50 group-hover:border-amber-200 transition-colors">
                          <CurrencyInput 
                            value={dayData.pieces} 
                            onChange={(val) => handleDataChange(day, 'pieces', val)} 
                            className="text-slate-700 font-medium"
                          />
                        </div>
                      </td>
                      <td className={`px-4 py-2 text-center font-bold ${ecartColor}`}>
                        {(misAuCoffre !== 0 || pieces !== 0 || especesTheorique !== 0) ? formatCurrency(ecart) : '-'}
                      </td>
                      <td className="px-2 py-2">
                        <input 
                          type="text" 
              ...
              <tfoot className="bg-slate-800 text-white font-bold sticky bottom-0 z-20">
                <tr>
                  ...
                  <td className="px-4 py-4 text-center text-blue-200">{formatCurrency(sumEspecesTheorique)}</td>
                  <td className="px-4 py-4 text-center">{formatCurrency(sumMisAuCoffre)}</td>
                  <td className="px-4 py-4 text-center">{formatCurrency(sumPieces)}</td>
                  <td className={`px-4 py-4 text-center ${sumTotalEcart < -0.001 ? 'text-rose-400' : sumTotalEcart > 0.001 ? 'text-emerald-400' : 'text-slate-400'}`}>
                    {formatCurrency(sumTotalEcart)}
                  </td>
