...
export const DEFAULT_ACCOUNTING_MAPPINGS: AccountingMappingRow[] = [
  ...
  row('especes', 'Paiement', 'totalTtc', 'Total TTC journée caisse', '531100', '', 'CAISSE', 'Débit 531100 = total TTC enregistré par la caisse. Ne pas confondre avec les espèces réelles.', 'TOTAL TTC; CAISSE TOTALE', 'Brouillon : première ligne 531100 du journal comptable.'),
  ...
  row('virement-interne', 'Ajustement', 'fondCaisse', 'Fond de caisse / Virement interne', '580000', '', 'CAISSE', 'Montant à confirmer avec le comptable. Valeur brouillon à 0 tant que la règle n’est pas validée.', '580; FOND CAISSE; VIREMENT INTERNE', 'À confirmer avant export officiel.'),
  ...
  row('pourboires', 'Ajustement', 'pourboires', 'Pourboires', '', '511280', 'POURBOIRE', 'Ligne de pourboire isolée du CA.', 'POURBOIRE; TIPS', 'Brouillon : ligne au crédit selon le modèle fourni.'),
  row('especes-remise-banque', 'Ajustement', 'especesRemise', 'Remise espèces / contrepartie 531100', '', '531100', 'CAISSE', 'Contrepartie 531100 vue en dernière ligne. Montant exact à confirmer avec le comptable.', 'REMISE BANQUE; 531100', 'Brouillon : valeur à 0 tant que la règle n’est pas validée.'),
];
...
export const normalizeAccountingMappings = (input: unknown): AccountingMappingRow[] => {
  ...
  const normalDefaults = DEFAULT_ACCOUNTING_MAPPINGS.map(defaultRow => {
    ...
    return { ...defaultRow, active: typeof saved.active === 'boolean' ? saved.active : defaultRow.active, category: cleanCategory(saved.category, defaultRow.category), caisseKey: defaultRow.caisseKey, caisseItem: saved.caisseItem || defaultRow.caisseItem, company: typeof saved.company === 'string' ? saved.company : defaultRow.company, debitAccount: typeof saved.debitAccount === 'string' ? saved.debitAccount : defaultRow.debitAccount, creditAccount: typeof saved.creditAccount === 'string' ? saved.creditAccount : defaultRow.creditAccount, accountingLabel: saved.accountingLabel || defaultRow.accountingLabel, rule: saved.rule || defaultRow.rule, tolerance: saved.tolerance || defaultRow.tolerance, keywords: typeof saved.keywords === 'string' ? saved.keywords : defaultRow.keywords, notes: typeof saved.notes === 'string' ? saved.notes : defaultRow.notes };
  });
