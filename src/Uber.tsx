...
import { useData, DayDataUber } from '@/contexts/DataContext';
...
export default function Uber({ month, year, onBack }: UberProps) {
  ...
  const handleDataChange = (day: number, field: keyof DayDataUber, value: string) => {
    updateUber(month, day, field, value);
  };
  ...
  const getVal = (day: number, field: keyof DayDataUber) => {
    const val = parseFloat((data[day]?.[field] || '').replace(',', '.'));
    return isNaN(val) ? 0 : val;
  };

  const getTheoriqueVal = (day: number, field: 'uber') => {
    const val = parseFloat((theoriqueData[day]?.[field] || '').replace(',', '.'));
    return isNaN(val) ? 0 : val;
  };

  const getColTotal = (field: keyof DayDataUber) => {
    return days.reduce((sum, day) => sum + getVal(day, field), 0);
  };

  const getTheoriqueColTotal = (field: 'uber') => {
    return days.reduce((sum, day) => sum + getTheoriqueVal(day, field), 0);
  };

  const sumUberTheorique = getTheoriqueColTotal('uber');
  const sumReel = getColTotal('reel');
  const sumEcartTTC = sumReel - sumUberTheorique;
            ...
            <table className="w-full text-sm text-left">
              <thead className="text-xs text-slate-500 uppercase bg-slate-50 border-b border-slate-200 sticky top-0 z-20">
                <tr>
                  ...
                  <th className="px-6 py-4 font-bold text-center w-48 bg-blue-50/50 border-r border-slate-200 text-blue-700">Uber Théorique</th>
                  <th className="px-6 py-4 font-bold text-center w-48 bg-amber-50/50 border-r border-slate-200 text-amber-700">Réel</th>
                  <th className="px-6 py-4 font-bold text-center w-48 border-r border-slate-200">Écart TTC Uber</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {days.map((day) => {
                  ...
                  const uberTheorique = getTheoriqueVal(day, 'uber');
                  const reel = getVal(day, 'reel');
                  
                  const ecart = reel - uberTheorique;
                  ...
                  return (
                    <tr key={day} className="hover:bg-slate-50/50 transition-colors group">
                      ...
                      <td className="px-6 py-3 text-right font-semibold text-slate-700 bg-blue-50/30 border-r border-slate-100">
                        {uberTheorique !== 0 ? formatCurrency(uberTheorique) : '-'}
                      </td>
                      ...
                      <td className={`px-6 py-3 text-right font-bold border-r border-slate-100 ${ecartColor}`}>
                        {(uberTheorique !== 0 || reel !== 0) ? formatCurrency(ecart) : '-'}
                      </td>
              ...
              <tfoot className="bg-slate-800 text-white font-bold sticky bottom-0 z-20">
                <tr>
                  ...
                  <td className="px-6 py-4 text-right text-blue-200 border-r border-slate-700">{formatCurrency(sumUberTheorique)}</td>
