...
export default function Deliveroo({ month, year, onBack }: DeliverooProps) {
  ...
  const handleDataChange = (day: number, field: keyof DayDataDeliveroo, value: string) => {
    updateDeliveroo(month, day, field, value);
  };
  ...
  const getVal = (day: number, field: keyof DayDataDeliveroo) => {
    const val = parseFloat((data[day]?.[field] || '').replace(',', '.'));
    return isNaN(val) ? 0 : val;
  };

  const getTheoriqueVal = (day: number, field: 'deliveroo') => {
    const val = parseFloat((theoriqueData[day]?.[field] || '').replace(',', '.'));
    return isNaN(val) ? 0 : val;
  };

  const getColTotal = (field: keyof DayDataDeliveroo) => {
    return days.reduce((sum, day) => sum + getVal(day, field), 0);
  };

  const getTheoriqueColTotal = (field: 'deliveroo') => {
    return days.reduce((sum, day) => sum + getTheoriqueVal(day, field), 0);
  };

  const sumTheorique = getTheoriqueColTotal('deliveroo');
            ...
            <table className="w-full text-sm text-left">
              <thead className="text-xs text-slate-500 uppercase bg-slate-50 border-b border-slate-200 sticky top-0 z-20">
                <tr>
                  ...
                  <th className="px-6 py-4 font-bold text-center w-48 bg-blue-50/50 border-r border-slate-200 text-blue-700">Deliveroo TTC Théorique</th>
                  <th className="px-6 py-4 font-bold text-center w-48 bg-amber-50/50 border-r border-slate-200 text-amber-700">Réel</th>
                  <th className="px-6 py-4 font-bold text-center w-48 border-r border-slate-200">Écart TTC Deliveroo</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {days.map((day) => {
                  ...
                  const theorique = getTheoriqueVal(day, 'deliveroo');
