...
- Dashboard.tsx decompose : `calculatedData` → `computeDashboardData` dans `dashboardCalculations.ts`, `dynamicColumns` → `buildDynamicColumns` dans `dashboardColumns.ts`, JSX → `DashboardTableHeader` + `DashboardTableBody` dans `components/`. Regle d'architecture posee dans `src/features/dashboard/ARCHITECTURE.md`. Dashboard.tsx passe de 2174 a 1139 lignes (objectif futur < 400 lignes). `tsc --noEmit` sans erreur.
