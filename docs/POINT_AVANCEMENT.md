...
- ROADMAP_STRUCTURE etape 6 (premier groupe) : migration `reel: string → number` pour Sunday, Uber, Deliveroo, ClickCollect. Parsing dans les updaters dedies via `parseMoneyValue`. `CurrencyInput` et `CanalSaisie` rendus tolerants `string | number`. `DashboardCaisseView` convertit via `reelStr()`. Donnees en storage existantes (string) continuent de fonctionner. 64 tests passent. La strategie est validee ; les groupes suivants (Nepting, Especes, Conecs, AmexAncv, AncvPapiers, BilanSynthese, DepensesPetiteCaisse, Theorique) restent a migrer.
