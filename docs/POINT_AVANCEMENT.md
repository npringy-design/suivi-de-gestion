...
- Etape C terminee : migration des parsings monetaires locaux (`n`, `parseVal`, `parseValue`, `parseNum`, fonctions inline) vers `parseMoneyValue` de `@/lib/money` dans `EdgMensuel`, `VsBudget`, `VsN1`, `Reporting`, `BudgetEdgAnnuel`, `RealiseEdgAnneeFiscale`, `MiseEnPaiement`, `ConfigSalaires`, `RecapAnnuel`, `ExportComptable`, `DashboardAnalysisView` et `DepensesPetiteCaisse`. Logique metier inchangee. `tsc --noEmit` inchange (memes 3 erreurs preexistantes dans `Dashboard.tsx`). Commit unique.
