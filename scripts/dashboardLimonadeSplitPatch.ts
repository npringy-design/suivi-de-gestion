...
export const dashboardLimonadeSplitPatch = (): Plugin => ({
  ...
  transform(code, id) {
    ...
    next = replaceRequired(
      ...
      "      return !isEditableColumn || contextColumns.has(colIndex);\n    });\n\n    const limonadeTechnicalColumns = new Set([34, 35, 36, 110, 111, 112, 113, 114, 115, 116, 117, 118]);\n    const limonadeVisibleOrder = [113, 114, 115, 116, 117, 118, 36];\n    const limonadeVisibleColumns = limonadeVisibleOrder\n      .map(colIndex => baseVisibleColumns.find(col => col.originalIndex === colIndex))\n      .filter(Boolean) as VisibleDashboardColumn[];\n\n    if (limonadeVisibleColumns.length === 0) return baseVisibleColumns;\n\n    const withoutLegacyLimonade = baseVisibleColumns.filter(col => !limonadeTechnicalColumns.has(col.originalIndex));\n    const insertAfterRestaurant = withoutLegacyLimonade.findIndex(col => col.originalIndex === 33);\n    if (insertAfterRestaurant === -1) return baseVisibleColumns;\n\n    return [\n      ...withoutLegacyLimonade.slice(0, insertAfterRestaurant + 1),\n      ...limonadeVisibleColumns,\n      ...withoutLegacyLimonade.slice(insertAfterRestaurant + 1),\n    ];\n  }, [activeTab, tableViewMode, dynamicColumns]);",
