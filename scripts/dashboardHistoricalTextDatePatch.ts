...
export const dashboardHistoricalTextDatePatch = (): Plugin => ({
  ...
  transform(code, id) {
    ...
    if (!code.includes(marker)) throw new Error('Patch date texte historique non applique');

    const insert = `    const normalizedTextDate = text.normalize('NFD').replace(/[\\u0300-\\u036f]/g, '').replace(/\\./g, ' ').replace(/\\s+/g, ' ').trim().toLowerCase();
    const monthWords = { janvier: 0, janv: 0, fevrier: 1, fevr: 1, fev: 1, mars: 2, avril: 3, avr: 3, mai: 4, juin: 5, juillet: 6, juil: 6, aout: 7, septembre: 8, sept: 8, octobre: 9, oct: 9, novembre: 10, nov: 10, decembre: 11, dec: 11 } as Record<string, number>;
