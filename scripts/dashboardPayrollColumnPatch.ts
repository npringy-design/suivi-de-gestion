...
export const dashboardPayrollColumnPatch = (): Plugin => ({
  ...
  transform(code, id) {
    ...
    next = mustReplace(next, "  const parseDashboardNumber = (value: string | number | undefined) => {", "  const isPayrollInputColumn = (colIndex: number) => (colIndex >= 62 && colIndex <= 71) || (colIndex >= 77 && colIndex <= 86);\n\n  const formatPayrollHourInputValue = (value: string | number | undefined) => {\n    if (value === undefined || value === null || value === '') return '';\n    const converted = parseHourInputToDecimal(value);\n    return converted > 0 ? converted.toFixed(2).replace('.', ',') : String(value);\n  };\n\n  const parseDashboardNumber = (value: string | number | undefined) => {", 'helpers conversion affichage heures personnel');
