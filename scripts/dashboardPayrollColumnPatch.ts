...
export const dashboardPayrollColumnPatch = (): Plugin => ({
  ...
  transform(code, id) {
    ...
    next = mustReplace(next, / {4}if \(isTextCol\) \{[\s\S]*? {4}\} else \{[\s\S]*? {6}updateDashboard\(month, `\$\{rIdx\}-\$\{cIdx\}`, cleanValue\);\r?\n {4}\}/, "    if (isTextCol) {\n      updateDashboard(month, `${rIdx}-${cIdx}`, value);\n    } else if (isPayrollInputColumn(cIdx)) {\n      const cleanHourValue = value.replace(/[^0-9hH:.,\\s]/g, '');\n      updateDashboard(month, `${rIdx}-${cIdx}`, cleanHourValue);\n    } else {\n      const cleanValue = value.replace(/[^0-9.,-]/g, '').replace(',', '.');\n      updateDashboard(month, `${rIdx}-${cIdx}`, cleanValue);\n    }", 'saisie heures personnel brute');

    next = replaceEvery(next, "const val = parseHourInputToDecimal(data[`${rIdx}-${colIdx}`] || '0');", "const val = parsePayrollHourForCalculation(data[`${rIdx}-${colIdx}`] || '0');", 'conversion centieme heures jour');
    ...
    next = mustReplace(next, / {4}if \(val === '' \|\| val === undefined \|\| val === null\) return '';\r?\n {4}\r?\n {4}\/\/ If the value already contains a percentage sign, return it as is/, "    if (val === '' || val === undefined || val === null) return '';\n\n    if (typeof colIndex === 'number' && isPayrollInputColumn(colIndex)) {\n      return formatPayrollHourDecimalValue(val);\n    }\n    \n    // If the value already contains a percentage sign, return it as is", 'format decimal heures personnel vue complete');
    next = mustReplace(next, "  const getDailyDisplayValue = (col: number) => formatValue(getDailyCellValue(col), dynamicColumns[col] || ['', '', '', '']);", "  const getDailyDisplayValue = (col: number) => {\n    const value = getDailyCellValue(col);\n    return isPayrollInputColumn(col) ? formatPayrollHourVisualValue(value) : formatValue(value, dynamicColumns[col] || ['', '', '', ''], col);\n  };", 'affichage heures saisie journaliere');
